...
\begin{align*}
   H &=  H_0 + H_\text{Def} + H_\text{Kop}\\
    &=- t  \sum_{\langle ij \rangle} \left ( c_i^\dagger c_j + c_j^\dagger c_i \right )  + \sum_{m=1}^5 \varepsilon_m d_m^\dagger d_m
...
\end{equation}
angesetzt werden.
Dabei sind $N$ die Anzahl der Einheitszellen, $\vec{l}_i$ der Gittervektor der Basis, $\vec{k}$ der Wellenvektor, $\vec{\delta}_j$ der Abstandsvektor
...
\begin{equation}
    ...
    \end{pmatrix} \label{eqn:gammakop}
\end{equation} 
gegeben.
