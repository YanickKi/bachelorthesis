...
\begin{equation*}
    ...
    n = \frac{\vec{d} \cdot \hat{z}}{\left | \vec{d} \right |}
\end{equation*}
gegeben sind.
...
\begin{align*}
    ...
    \tilde{d}_1 &= \frac{1}{\sqrt{2}} (d_2+\symup{i}d_3) & \tilde{d}_3 &= \frac{1}{\sqrt{2}} (d_2-\symup{i}d_3) \; .   \numberthis \label{eqn:newdi}
\end{align*}
...
\end{align}
Dabei sieht $\underline{\underline{\gamma}}$ wie folgt aus:
\begin{equation*}
...
%        0                       & 0                     & \symup{\Delta}_{32}   & \symup{\Delta}_{33}   & 0                     \\
%        0                       & 0                         & 0                     & 0                 & \symup{\Delta}_{44}   \\
%    \end{pmatrix}
%\end{equation*}
\section{Andere Variante}
Der Hamiltonian des ungestörten Graphens ist
\begin{equation*}
    - t \sum_{i=1}^N \sum_{j=1}^N\left ( c_i^\dagger c_j + c_j^\dagger c_i \right ) \; .
\end{equation*}
Mit den Fouriertransformationen für $c_i$ und $c_j$ 
\begin{equation}
    c_i = \frac{1}{\sqrt{N}} \sum_{\vec{k}} \symup{e}^{i\vec{k}\vec{l}} c_{\text{A}, \vec{k}} \; , 
    \quad c_j = \frac{1}{\sqrt{N}} \sum_{\vec{k}} \symup{e}^{i\vec{k}(\vec{l}+\vec{\delta}_j)} c_{\text{B}, \vec{k}} 
\end{equation}
ergibt sich der Hamiltonian mit Vereinfachungen zu
\begin{equation}
...
\end{equation} 
Der Vektor $\symup{\delta}_j$ ist dabei der Abstandsvektor des nächsten Nachbarn auf dem Untergitter B.
Nun die Greensfunktionen mit Bewegungsleichung ausrechnen, wobei erst die Kommutatoren
\begin{align}
    \left [H,  c_{\text{B},\vec{k}} \right ] &= t \sum_j \symup{e}^{-\symup{i}\vec{k}\vec{\delta}_j} c_{\text{A},\vec{k}} \\
    \left [H,  c_{\text{A},\vec{k}} \right ] &= t \sum_j \symup{e}^{\symup{i}\vec{k}\vec{\delta}_j} c_{\text{B},\vec{k}}
\end{align}
benötigt werden. 
Damit erhält man die Greensfunktionen:
\begin{align}
    G_{c_{\text{A},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} &=  \frac{\sum_j \symup{e}^{\symup{i}\vec{k}\vec{\delta}_j} G_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}}}{z}\\
    zG_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} &= 1 -  \sum_j \symup{e}^{-\symup{i}\vec{k}\vec{\delta}_j} G_{c_{\text{A},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} \; .
\end{align}
Die Greensfunktion $G_{c_{\text{A},\vec{k}}, c^\dagger_{\text{B},\vec{k}}}$ in $G_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}}$ einsetzten:
\begin{align}
    zG_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} &= 1 - \sum_j \symup{e}^{-\symup{i}\vec{k}\vec{\delta}_j} \frac{\sum_{j'} \symup{e}^{\symup{i}\vec{k}\vec{\delta}_{j}} G_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}}}{z}\\
    \iff G_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} &= \frac{z}{z^2 - \varepsilon^2_{\vec{k}}  } \; .
\end{align}
Dabei ergibt sich die Dispersionsrelation von Graphen mit $\varepsilon^{\pm}_{\vec{k}} = \pm  t \sum_j\symup{e}^{\symup{i}\vec{k}\vec{\delta}_j} \sum_{j'}\symup{e}^{-\symup{i}\vec{k}\vec{\delta}_{j'}}$.
Jetzt 
\begin{equation}
    G_{\tilde{c}_i, \tilde{c}^\dagger_j} = \sum_{\vec{k}} D_i D^\dagger_j G_{c_{\text{B},\vec{k}}, c^\dagger_{\text{B},\vec{k}}} \label{eqn:koe}
\end{equation}
ausnutzen, wobei $D_i$ und $D_j$ die Koeffizienten von den Fouriertransformationen 
\begin{align}
...
\begin{align}
    ...
    D_2 &= \frac{1}{\sqrt{3N}}\sum_{j=1}^3\symup{e}^{i{\vec{k}}(\vec{l}+\vec{\delta}_j)-\symup{i}\frac{2(j-1)\pi}{3}} \; .
\end{align}
Werden die Koeffizienten $D_i$ nun in \eqref{eqn:koe} eingesetzt, ergibt sich
\begin{align}
...
\end{align}
Mit der Matrix $\underline{\underline{\gamma}}$
\begin{equation*}
    (\underline{\underline{\gamma}})_{ml} = 
    \begin{pmatrix}
        0               &\gamma_{01}    &   0           \\
        0               &\gamma_{11}   &   0           \\
        0               &0              &   \gamma_{22} \\
    ...
    \end{pmatrix}
\end{equation*}
dessen Einträge die Vorfaktoren aus dem Hamiltonian $H_\text{Kop}$ \eqref{eqn:Hkopnice} mit den neuen $\tilde{c_l}$ und $\tilde{d}_m$ sind,
sieht $\underline{\underline{\symup{\Delta}}}$ wie folgt aus:
\begin{equation}
