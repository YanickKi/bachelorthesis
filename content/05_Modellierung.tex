...
\begin{align*}
   H &=  H_0 + H_\text{Def} + H_\text{Kop}\\
    &=- t  \sum_{\langle ij \rangle}^N
    \left ( c_i^\dagger c_j + c_j^\dagger c_i \right )  + \sum_{m=1}^5 \varepsilon_m d_m^\dagger d_m
...
Der Vernichter(Erzeuger) $c_i(c_i^{\dagger})$ vernichtet(erzeugt) ein Elektron im $p_z$-Orbital 
eines C am Gitterplatz $i$.
...
Dabei sitzt das Mn auf dem Untergitter A mit dem Gittervektor $\vec{l}$, weswegen die Summe über $j$ mit $j \in \{1,2,3 \}$ über die nächsten Nachbarn 
...
$(l,m,n) \to (-l,-m,-n)$, wie in Abschnitt \ref{sec:theo_SK} beschrieben, durchgeführt werden muss, da die Abstandsvektoren $d_j$ aus Gleichung \eqref{eqn:disvec} von 
Dem C zu dem Mn zeigen müssen.
...
\begin{equation}
    ...
    \quad c_j = \frac{1}{\sqrt{N}} \sum_{\vec{k}}^{1. \, \text{BZ}} \symup{e}^{\symup{i}\vec{k}(\vec{l}_i+\vec{\delta}_j)} c_{\text{B}, \vec{k}} \label{eqn:fourier_ladder}
\end{equation}
angesetzt werden.
Dabei sind $N$ die Anzahl der Einheitszellen, $\vec{l}_i$ der Gittervektor der Einheitszelle, $\vec{k}$ der Wellenvektor, $\vec{\delta}_j$ der Abstandsvektor
des $j$-ten C auf dem Untergitter B, welches eins der drei nächsten Nachbarn des C auf dem Untergitter A mit dem Gittervektor $\vec{l}_i$ ist 
...
\begin{align*}
    ...
    \symup{e}^{i\vec{k}\vec{l}_i}c_{A,\vec{k}}) \\
...
\end{equation}
bestimmt werden.
Da sich das Argument der Greenschen Funktion nicht ändert, wird im Folgenden drauf verzichtet, das Argument mit anzugeben.
...
Ebenfalls wird die Änderung der Notation für die Einträge der Matrix $\underline{\underline{G}}$
\begin{equation*}
...
Diese Symmetrie ist in den SK-Integralen enthalten, weswegen diese explizit in den Hamiltonian eingesetzt werden.
Dafür müssen die Vernichter $c_i$ zunächst in eine neue Basis transformiert werden.
...
\begin{equation}
    ...
    c_2 &= \frac{1}{\sqrt{3}}\left (\tilde{c}_0 + \symup{e}^{\symup{i}\frac{4\pi}{3}} \tilde{c}_1 + \symup{e}^{\symup{i}\frac{2\pi}{3}} \tilde{c}_2 \right ) \\
    c_2 &= \frac{1}{\sqrt{3}}\left (\tilde{c}_0 + \symup{e}^{\symup{i}\frac{2\pi}{3}} \tilde{c}_1 + \symup{e}^{\symup{i}\frac{4\pi}{3}} \tilde{c}_2 \right ) 
...
\end{equation} 
Nun werden in Gleichung \eqref{eqn:lcinhamiltonian} neue Linearkombinationen der Vernichter $d_i$ ersichtlich.
Diese lauten 
\begin{equation}
    \begin{aligned}
...
\begin{wrapfigure}{r}{0.35\textwidth}
    ...
    \label{fig:3band}
\end{wrapfigure}
In Gleichung \eqref{eqn:Hkopnice} zeigt sich welche Operatoren $\tilde{d}_m^{\dagger}$ an welche Vernichter $\tilde{c}_l$ koppeln.
An den Vernichter $\tilde{c}_{0}$ koppelt nur ein einziger Erzeuger, $\tilde{d}_{4}^{\dagger}$, da das Orbital $d_{z^2}$ invariant unter einer Drehung ist,
...
\\
\begin{align}
    H = -t\sum_{\langle ij\rangle}^N(c^\dagger_i c_j + c^\dagger_j c_i) + \varepsilon \sum_{m=0}^4 \tilde{d}^\dagger_m \tilde{d}_m +
...
\end{equation}
gegeben.
Werden diese nun in \eqref{eqn:koe} eingesetzt, ergeben sich die drei gesuchten Greenschen Funktionen zu 
\begin{align}
...
%\end{equation}
\[
    % \def\nullblock{\begin{matrix}0&0\\0&0\end{matrix}}
...
% 
    % \def\nullblock{
    %     \begin{matrix}
    %         \rlap{0}\phantom{\gamma_{01}^2} & 
    %         \rlap{0}\phantom{\gamma_{01}\gamma_{11}} \\
    %         \rlap{0}\phantom{\gamma_{01}\gamma_{11}} & 
    %         \rlap{0}\phantom{gamma_{11}^2}
    %     \end{matrix}
    %     }
% 
