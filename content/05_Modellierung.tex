...
\end{equation*}
für die Einträge der Matrix $\underline{\underline{G}}$, welche die gesuchten Greenschen Funktionen sind, durchgeführt.
...
\end{equation} 
Nun werden in Gleichung \eqref{eqn:lcinhamiltonian} neue Linearkombinationen der Vernichter $d_m$ ersichtlich.
Diese lauten 
\begin{equation}
    \begin{aligned}
...
\end{align}
folgt. 
Dabei tritt die Dispersionsrelation von Graphen $\varepsilon_{\vec{k}}$ (Berechnung in Abschnitt \ref{sec:calc_dispersion}) auf. 
...
durch Multiplikation der Kopplungsparameter ${\gamma_{ml} \in \{\xi, \chi, \zeta \}}$ (Gl. \ref{eqn:gammakopcrazy}) mit der zu dem $m$ zugehörige Greensche Funktion $G_{\tilde{c}_l, \tilde{c}^\dagger_{l'}}$ gegeben sind.
...
Daraus kann gefolgert werden, dass der Anteil der Kopplungen ausgehend von den $\pi$-Bindungen bei dem SK-Integral $E_{z,zy}$ für die C, deren
