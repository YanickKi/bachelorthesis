...
\end{align*} 
Dabei ist  $\symup{\delta} (\tau-\tau') = \symup{\delta} (\tau'-\tau) $ die Deltadistribution als Ableitung der Heaviside-Funktion.
...
Der Vektor $\vec{R}_{\alpha}$ ist die Position innerhalb der Einheitszelle mit dem Gittervektor $\vec{l}_j$, während
...
Dabei vernichtet(erzeugt) der Operator $c_{jlm\alpha}$($c_{jlm\alpha}^{\dagger}$ ) ein Elektron in einem Orbital mit den Drehimpulsquantenzahlen $l$ und $m$ in der Einheitszelle $j$,
innerhalb der Einheitszelle am Platz $\alpha$.
